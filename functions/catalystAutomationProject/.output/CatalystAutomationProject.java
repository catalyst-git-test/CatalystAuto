...
public class CatalystAutomationProject implements CatalystAdvancedIOHandler {
	...
	private static Long testTableId=1926000004342810L;
	private static Long expectedColumnIdTestTable=1926000004342813L;
	static String GET = "GET";
	static String POST = "POST";
	private static String TABLENAME2 = "FKTable";
	private static Long expectedColumnIdFKTable=1926000004343532L;
	String name = "";
		...
		try {
			if (method.equals(POST)) {
				...
				for (int i = 0; i < rowList.size(); i++) {
					...
					if (var.equals("qq")) {
						value += "Expected value of var is qq and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of var is qq but the value obtained is " + var
								+ ". Values mismatch !<br> ";
						flag = true;
					}
					if (mandtext.equals("~!@#$%^&*()_+{}|<>?:';,./[]\\`")) {
						value += "Expected value of mandtext is ~!@#$%^&*()_+{}|<>?:';,./[]\\` and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of mandtext is ~!@#$%^&*()_+{}|<>?:';,./[]\\` but the value obtained is "
								+ mandtext + ". Values mismatch !<br>";
						flag = true;
					}
					if (integerUnique.equals("999999999")) {
						value += "Expected value of interunique is 999999999 and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of integerunique is 999999999 but the value obtained is "
								+ integerUnique + ". Values mismatch !<br>";
						flag = true;
					}
					if (defaultDateVal.equals("2021-05-01")) {
						value += "Expected value of defaultDateVal is 2021-05-01 and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of defaultDateVal is 2021-05-01 but the value obtained is "
								+ defaultDateVal + ". Values mismatch !<br>";
						flag = true;
					}
					if (doubleVal.equals(1.23)) {
						value += "Expected value of doubleVal is 1.23 and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of doubleVal is 1.23 but the value obtained is " + doubleVal
								+ ". Values mismatch !<br>";
						flag = true;
					}
					if (bigintVal.equals("-11")) {
						value += "Expected value of bigintVal is -11 and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of bigintVal is -11 but the value obtained is " + bigintVal
								+ ". Values mismatch !<br>";
						flag = true;
					}
					if (encVal.equals("1234")) {
						value += "Expected value of encVal is 1234 and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of encVal is 1234 but the value obtained is " + encVal
								+ ". Values mismatch !<br>";
						flag = true;
					}
					if (dateTime.equals("2021-03-01 23:59:59")) {
						value += "Expected value of dateTime is 2021-03-01 23:59:59 and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of dateTime is 2021-03-01 23:59:59 but the value obtained is "
								+ dateTime + ". Values mismatch !<br>";
						flag = true;
					}
					if (boolVal.equals("false")) {
						value += "Expected value of boolVal is false and the value is fetched correctly.<br>";
					} else {
						value += "Expected value of boolVal is false but the value obtained is " + boolVal
								+ ". Values mismatch !<br>";
						flag = true;
					}
		...
		try {
			if (method.equals(POST)) {
				...
				responseData.put("message",
						"Duplicate check exception was not thrown and duplicate value has been inserted. Case Failed");
				responseData.put("flag", flag);
				response.setContentType("application/json");
				response.getWriter().write(responseData.toString());
			...
			if (name.contains("app")) {
				...
				if (exceptionAsString.contains("Duplicate value")) {
					responseData.put("message",
							"Duplicate value found for one of the entries alert is correctly thrown. Case Passed");
					responseData.put("flag", flag);
					response.setContentType("application/json");
					response.getWriter().write(responseData.toString());
					response.setStatus(200);
	...
	public void testCase4(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (name.contains("app")) {
				...
				if (exceptionAsString.contains(
						"com.zc.exception.ZCServerException. Caused by : Invalid input value for integerUnique. int value expected INVALID_INPUT")) {
	...
	public void testCase5(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (name.contains("app")
					&& e.toString().contains("com.zc.exception.ZCServerException. Caused by : UnAuthorized")) {
				responseData.put("message", "Expected exception is thrown for app user");
				responseData.put("flag", flag);
				response.setContentType("application/json");
				response.getWriter().write(responseData.toString());
	...
	public void testCase7(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (mandVal.equals("Value1") && varText.equals("multiple") && doubleVal.equals("3.45")
					&& mandVal2.equals("Value2") && intUnique.equals("100")) {
	...
	public void testCase9(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			expValues.expectedId = expectedColumnIdTestTable;
	...
	public void testCase10(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			ColumnData expValues = new ColumnData();
			expValues.expectedId = expectedColumnIdFKTable;
			...
			if (col.getParentColumn() != null) {
				...
				if (!parentColumn.equals(expectedColumnIdTestTable.toString())) {
					flag = true;
					resp.put("flag", flag);
					val = resp.get("message").toString();
					resp.put("message", val
							+ ". Parent Column value mismatch for foreign key column. Expected - "+expectedColumnIdTestTable+", Found - "
							+ parentColumn);
				} else {
					val = resp.get("message").toString();
			...
			if (col.getParentTable() != null) {
				...
				if (!parentTable.equals(testTableId.toString())) {
					flag = true;
					resp.put("flag", flag);
					val = resp.get("message").toString();
					resp.put("message", val
							+ ". Parent Table value mismatch for foreign key column. Expected - "+testTableId+", Found - "
							+ parentTable);
				} else {
					val = resp.get("message").toString();
	...
	public JSONObject columnDetailsCheck(ZCColumn col, ColumnData expValues) {
		...
		try {
			...
			if (id.equals(expValues.expectedId) && name.equals(expValues.expectedName)
					&& type.equals(expValues.expectedType) && Boolean.compare(mand, expValues.expectedMand) == 0) {
				responseData.put("message", "Expected values for the column properties are obtained");
			} else {
				flag = true;
				responseData.put("message",
						"Mismatch in one of the properties. Expected values - <br>column id - " + expValues.expectedId
								+ " <br>column name -  " + expValues.expectedName + " <br>column type - "
								+ expValues.expectedType + " <br>is mandatory value - " + expValues.expectedMand
								+ "<br> Found : column id - " + id + " <br>column name - " + name
								+ " <br>column type - " + type + " <br>is mandatory value - " + mand + "<br>");
			}
			responseData.put("flag", flag);
			// responseData.put("details",col.getColumnId()+" "+col.getColumnName()+"
			// "+col.getDataType()+" "+col.getIsMandatory());
			return responseData;
		} catch (Exception e) {
			responseData.put("message", e.toString());
			responseData.put("value", "exception");
